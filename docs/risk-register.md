...
| R-003 | RLS incorreto permitir acesso cruzado | Critico | Media | Policies por `auth.uid()` e harness `npm run test:rls:catalog`; falta execucao com usuarios reais | Aberto |
...
| R-012 | Catalogo compartilhado conflitar com RLS por usuario | Alto | Media | Modelo RLS versionado e harness criado; falta rodar com `RLS_TEST_*` | Em mitigacao |
